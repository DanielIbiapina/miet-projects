...
import { mockUser, mockStickerDevice, simulatePassiveMeasurement, calculateProjection, getBACRange } from "../utils/mockData";
...
export default function HomeScreen() {
  ...
  const [deviceStatus, setDeviceStatus] = useState(mockStickerDevice.isAttached);
  ...
  useEffect(() => {
    if (readings.length > 0) {
      ...
      // Em Portugal, limite legal é 0.5 (0.05 BAC), então só avisar quando realmente elevado
      if (currentBAC >= 0.10) {
        setShowWarning({
          type: "high",
          message: "Your BAC level is significantly elevated. Do not drive. Arrange safe transportation immediately.",
        });
      } else if (currentBAC >= 0.08 && !showWarning) {
        setShowWarning({
          type: "moderate",
          message: "Your BAC level is above legal driving limit. Plan your safe ride home.",
        });
      } else if (currentBAC < 0.08) {
        // Remover aviso se BAC baixou
        setShowWarning(null);
      }
    }
  }, [readings]);
  ...
  const handleSimulateNight = () => {
    ...
    const userProfile = savedProfile ? JSON.parse(savedProfile) : mockUser.profile;
    ...
    const addMeasurement = () => {
      if (measurementIndex >= intervals.length) {
        ...
        setTimeout(() => {
          alert(`Night completed! Data saved to history. Starting new day.`);
        }, 500);
        return;
      }

      const timestamp = new Date(startTime);
      timestamp.setMinutes(startTime.getMinutes() + intervals.slice(0, measurementIndex + 1).reduce((a, b) => a + b, 0));
      
      const newReading = simulatePassiveMeasurement(userProfile, currentReadings);
    ...
    // Adicionar medições a cada ~10 segundos
  ...
  const handleShareResults = () => {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(`Current BAC: ${lastReading?.bac.toFixed(3)} (${lastReading?.level})`);
  ...
  const chartReadings = readings.slice(-8).sort((a, b) => 
    new Date(a.timestamp) - new Date(b.timestamp)
  );
  const maxBAC = Math.max(...chartReadings.map(r => r.bac || 0), 0.1);
  ...
  return (
    <Container>
      <SafeArea>
        ...
        <MainContent>
          {!readings.length && (
            <SimulateButton onClick={handleSimulateNight} $isSimulating={isSimulating}>
          ...
          {lastReading ? (
            <>
              <CurrentStatusCard>
                ...
                <StatusLabel style={{ marginTop: "0.5rem", fontSize: "0.875rem", opacity: 0.7 }}>
                    ...
                    <LineChart>
                      <ChartSVG viewBox={`0 0 ${chartWidth} ${chartHeight + padding * 2}`} preserveAspectRatio="xMidYMid meet">
                        ...
                        {chartReadings.length > 1 && (
                          <ChartLine
                            points={chartReadings.map((reading, index) => {
                              const x = padding + (index * (chartWidth - padding * 2) / (chartReadings.length - 1));
                              const y = chartHeight + padding - (reading.bac * chartHeight / maxBAC);
                              return `${x},${y}`;
                            }).join(' ')}
                            stroke={getLevelColor(chartReadings[chartReadings.length - 1].level)}
                            fill="none"
                            strokeWidth="3"
                          />
                        )}
                        ...
                        {chartReadings.map((reading, index) => {
                          const x = padding + (index * (chartWidth - padding * 2) / Math.max(chartReadings.length - 1, 1));
                          const y = chartHeight + padding - (reading.bac * chartHeight / maxBAC);
                          return (
                            <ChartPoint
              ...
              {projection && (
                <ProjectionCard>
                  ...
                  {projection.canDriveNow ? (
                    <>
                      <ProjectionValue $color="#10b981" style={{ fontSize: "1.5rem", marginBottom: "1rem" }}>
                        You can drive now
                      </ProjectionValue>
                    ...
                    <>
                      <ProjectionLabel style={{ marginBottom: "1rem", fontSize: "0.9375rem" }}>
                        If you stop drinking now and stay hydrated:
                      </ProjectionLabel>
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        <div>
                          <ProjectionLabel style={{ fontSize: "0.875rem", opacity: 0.9, marginBottom: "0.25rem" }}>
                            Safe to drive at:
                          </ProjectionLabel>
                          <ProjectionValue $color="#f59e0b" style={{ fontSize: "1.75rem" }}>
                        ...
                        <div>
                          ...
                          <ProjectionValue $color="#10b981" style={{ fontSize: "1.75rem" }}>
              ...
              <StatsGrid>
                ...
                <StatCard>
                  <StatValue>{lastReading.location.address.split(',')[0]}</StatValue>
                  ...
                  <ReadingsList>
                    {recentReadings.map((reading) => (
                      <ReadingItem key={reading.id}>
                        <ReadingTime>{formatTime(reading.timestamp)}</ReadingTime>
                        <ReadingBAC $color={getLevelColor(reading.level)}>
                          {reading.bac.toFixed(3)} BAC
                        </ReadingBAC>
